...
const narrative = [
  ...
  },
  {
    ...
    image: "/images/shared/shared-asset-1.jpeg",
  },
  {
    id: "03",
...
export function StorySection() {
  return (
    <section id="story" className="py-16 md:py-32 bg-carbon relative overflow-hidden border-b border-white/5">
      <Container className="relative z-10 max-w-[1800px] px-6 md:px-12 lg:px-24">
        ...
        <div className="mb-16 md:mb-24 flex flex-col items-center text-center">
          <div className="flex items-center gap-4 mb-6 justify-center">
            ...
            <Mono className="text-gold uppercase tracking-widest text-sm">The Methodology</Mono>
            <span className="w-8 h-[1px] bg-royal" />
          </div>
          <H2 className="text-4xl md:text-5xl lg:text-6xl leading-[1.1] max-w-4xl">Engineering from <br/>First Principles.</H2>
        </div>
        ...
        <div className="flex flex-col border-t border-white/10">
          {narrative.map((item, index) => {
            ...
            return (
              <div key={item.id} className="py-12 md:py-24 border-b border-white/10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
                
                {/* Image Side */}
                <div className={cn("relative w-full aspect-[4/3] lg:aspect-square overflow-hidden bg-obsidian rounded-sm order-1", isEven ? "lg:col-span-5 lg:order-1" : "lg:col-span-5 lg:order-2")}>
                  <Image src={item.image} alt={item.title} fill sizes="(max-width: 1024px) 100vw, 40vw" className="object-cover opacity-90 transition-transform duration-1000 hover:scale-105" />
                ...
                {/* Text Side */}
                <div className={cn("flex flex-col gap-6 order-2", isEven ? "lg:col-span-7 lg:order-2 lg:pl-12" : "lg:col-span-7 lg:order-1 lg:pr-12")}>
                  <div className="flex items-baseline gap-4 mb-2">
                    <Meta className="text-gold">[{item.id}]</Meta>
                    <Mono className="text-white/40 uppercase tracking-widest text-sm">{item.phase}</Mono>
                  </div>
                  <H3 className="text-3xl lg:text-5xl font-display tracking-tight text-white">{item.title}</H3>
                  <Body className="text-lg lg:text-xl text-white/60 leading-relaxed max-w-2xl">{item.desc}</Body>
                </div>
