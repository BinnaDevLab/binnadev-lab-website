    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {currentProjects.map((project) => (
          <div key={project.id} className="group overflow-hidden rounded-xl border border-white/5 bg-carbon hover:border-royal/50 transition-all duration-300 flex flex-col cursor-pointer">
              ...
              <div className="flex items-center gap-4 mt-auto">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.technologies.slice(0, 3).map((tech) => (
                    <span key={tech} className="text-xs font-mono text-white/60 bg-white/5 px-2 py-1 rounded">
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 3 && (
                    <span className="text-xs font-mono text-white/40 px-1 py-1">+{project.technologies.length - 3}</span>
                  )}
                </div>
              </div>
            </div>
