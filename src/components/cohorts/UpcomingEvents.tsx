...
export function UpcomingEvents({ events }: UpcomingEventsProps) {
  ...
  return (
    <section className="mb-40 relative z-10">
      <Container>
        <div className="mb-16">
          <Mono className="text-gold mb-4 block tracking-[0.2em] uppercase text-xs">Horizon</Mono>
          <H2 className="text-3xl md:text-5xl tracking-tight">Upcoming Events.</H2>
        </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                  <div className="lg:col-span-7 flex flex-col justify-center">
                    <Mono className="text-gold/80 text-xs uppercase tracking-[0.2em] mb-4 block">{evt.category}</Mono>
                    <H3 className="text-4xl md:text-5xl text-white mb-4">{evt.title}</H3>
                    <div className="text-2xl text-white/50 font-display italic mb-8">{evt.subtitle}</div>
                  ...
                  <div className="lg:col-span-5 flex flex-col gap-6 p-8 bg-carbon border border-white/5 rounded-2xl shadow-xl justify-center">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h4 className="text-white font-medium mb-1 text-sm tracking-wide">Date & Time</h4>
                        <p className="text-base text-white/60">{new Date(evt.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</p>
                      </div>
                    </div>
                    ...
                    {evt.location && (
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="text-white font-medium mb-1 text-sm tracking-wide">Location</h4>
                          <p className="text-base text-white/60">
                            {evt.location.type === 'in-person' ? evt.location.venue : evt.location.platform || 'Online'}
                          </p>
                          {evt.location.type === 'in-person' && (
                            <p className="text-sm text-white/40 mt-1">{evt.location.address}</p>
                          )}
                        </div>
                      </div>
                    )}
                    ...
                    {evt.speakers && evt.speakers.length > 0 && (
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <h4 className="text-white font-medium mb-1 text-sm tracking-wide">Featuring</h4>
                          <p className="text-base text-white/60">{evt.speakers.join(", ")}</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {evt.location.latitude && evt.location.longitude ? (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 z-0">
                        ...
                        <H3 className="text-xl text-gray-800 mb-2">Resolving Location</H3>
                        <Body className="text-gray-500 text-sm max-w-xs">Geographic coordinates pending...</Body>
                      </div>
                    )}
                  </motion.div>
                )}
